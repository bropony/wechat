...
CDateTimeException::CDateTimeException(const char * msg)
:CException(msg, 0)
{
}
...
*/
CDateTime::CDateTime()
	:_timeZone(0),_showZone(0),_tm(nullptr)
{
	std::time_t currentTime = std::time(nullptr);
	long64_t mills = long64_t(currentTime) * 1000;
...
void CDateTime::parse(const char* value, const char* format)
{
			...
			case 'n' :
				{
					...
					{
						G_THROW_EXCEPTION( CDateTimeException , "CDateTime Format Error!" << value );
					}
				};
			break;
			default :
	...
	if ( timeV == -1 )
	{
		G_THROW_EXCEPTION( CDateTimeException , "CDateTime Value Error!" << value );
	}
	long64_t mill = timeV;
...
long64_t CDateTime::getTotalSecond() const
{
	return (long64_t)(_timeSpan / 1000);
}
